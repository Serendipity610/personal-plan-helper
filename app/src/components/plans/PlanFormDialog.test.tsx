...
function renderDialog(plan: Plan | null = null, defaultTitle?: string) {
  return render(<PlanFormDialog onOpenChange={vi.fn()} plan={plan} defaultTitle={defaultTitle} />);
}
...
beforeEach(() => {
  ...
  mockedApi.createPlan.mockImplementation(async (input) =>
    makePlan({
      ...
      ddl: input.ddl ?? null,
      period_type:
        (input.period_type as "daily" | "monthly" | "quarterly" | "yearly" | null) ?? null,
...
describe("PlanFormDialog quick capture (collapsed mode for new plans)", () => {
  it("uses defaultTitle as the initial title for new plans", () => {
    renderDialog(null, "命令面板新计划");

    expect(screen.getByLabelText("标题")).toHaveValue("命令面板新计划");
  });
  ...
  it("preserves input on backend failure and allows retry", async () => {
    ...
    mockedApi.createPlan.mockResolvedValueOnce(makePlan({ id: "retry-plan", title: "失败的计划" }));
    await user.click(screen.getByRole("button", { name: "创建" }));
